         ...
         <div className="max-w-[1600px] mx-auto px-4 py-4 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
               <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-none flex flex-col justify-between">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{t('stockReport.liveInventory')}</span>
                  <span className="text-[13px] font-bold font-sans text-slate-800 mt-1">{formatNumber(totalValue.current)}</span>
               </div>
               <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-none flex flex-col justify-between">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{t('stockReport.grossProcurement')}</span>
                  <span className="text-[13px] font-bold font-sans text-slate-800 mt-1">{formatNumber(totalValue.purchased)}</span>
               </div>
               <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-none flex flex-col justify-between">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{t('stockReport.fulfilledSales')}</span>
                  <span className="text-[13px] font-bold font-sans text-slate-800 mt-1">{formatNumber(totalValue.sold)}</span>
               </div>
               <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-none flex flex-col justify-between">
                  <span className="text-[9px] font-bold text-red-500 uppercase tracking-widest">{t('stockReport.criticalReorders')}</span>
            ...
            <div className="bg-white border border-slate-200 rounded-lg overflow-hidden flex flex-col min-h-[600px] relative shadow-none">
               <div className="px-3.5 py-2.5 bg-slate-50 border-b border-slate-200 flex items-center justify-between flex-wrap gap-2 select-none">
                  <div className="flex items-center gap-3">
                     <div className="flex items-baseline gap-2">
                        <span className="text-xs font-extrabold text-slate-800 uppercase tracking-wider">{t('stockReport.audit')}</span>
                        <span className="bg-slate-200 text-slate-600 font-bold force-en text-[9px] px-1.5 py-0.5 rounded-sm">{filteredData.length} records</span>
                     </div>
                  </div>
               ...
               <div className="p-3.5 flex flex-col sm:flex-row sm:items-end gap-3 border-b border-slate-100 select-none">
                  ...
                  <button onClick={() => setFilterStatus('LOW')} className="h-7 flex items-center gap-1.5 px-2.5 bg-[#1d5f84] hover:bg-[#154662] border border-[#1d5f84] text-white text-[11px] font-bold rounded-md transition shadow-sm cursor-pointer uppercase tracking-wider">
               ...
               {lowStockData.length > 0 && (
                  <div className="mx-3.5 mt-3.5 bg-rose-50 border border-rose-200 rounded-md px-4 py-3 flex items-center justify-between gap-3">
                     <div className="flex items-center gap-3">
                        ...
                        <div>
                           <h3 className="text-xs font-extrabold text-rose-700 uppercase tracking-widest">Critical Procurement Alert</h3>
                        ...
                        {loading ? (
                           <tr>
                              <td colSpan="8" className="py-24 text-center">
                                 ...
                                 <p className="text-slate-400 font-bold uppercase text-[9px] tracking-widest italic">{t('saleReport.syncingStreams') || 'Fetching Statement Data...'}</p>
                              </td>
                           </tr>
                              ...
                              <tr key={idx} className="group hover:bg-slate-50/80 transition-colors border-b border-slate-100">
                                 <td className="px-3 py-1.5 border-r border-slate-100 leading-tight">
                                    <div className="flex flex-col gap-0.5">
                                       <span className="font-bold text-slate-800 text-[11px] uppercase tracking-tight">{item.item_name}</span>
                                       <span className="text-[9px] text-slate-400 font-mono">CODE: #{item.item_code}</span>
                                    </div>
                                 </td>
                                 <td className="px-3 py-1.5 border-r border-slate-100">
                                    <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider border border-slate-200 bg-slate-50 text-slate-700">{item.category || 'N/A'}</span>
                                 </td>
                                 ...
                                 <td className="px-3 py-1.5 border-r border-slate-100 text-center">
                                    <span className={`px-2 py-0.5 border text-[9px] font-bold uppercase tracking-wider rounded-md ${item.stock_status === 'LOW' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>{item.stock_status === 'LOW' ? t('stockReport.critical') : t('stockReport.optimal')}</span>
                                 </td>
               ...
               <div className="bg-slate-50 border-t border-slate-200 px-3.5 py-2.5 flex flex-col sm:flex-row justify-between items-center text-[9px] font-bold text-slate-400 uppercase tracking-wider gap-2 select-none">
                  <div className="flex items-center gap-3">
         ...
         {showHistory && selectedItem && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-[1px] select-none">
               <div className="relative w-full max-w-5xl bg-white border border-slate-200 rounded-lg shadow-none overflow-hidden flex flex-col max-h-[85vh] z-10">
                  ...
                  {/* Header Subtitle bar */}
                  <div className="bg-slate-50 border-b border-slate-200 px-4 py-2 flex flex-col font-mono text-xs text-slate-600">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 bg-white">
                     {itemHistory.length === 0 ? (
                        <div className="py-16 text-center text-slate-400 uppercase tracking-widest font-mono text-xs">
                                 ...
                                 {itemHistory.map((h, i) => (
                                    <tr key={i} className="hover:bg-slate-50 transition-colors">
                                       ...
                                       <td className="px-3 py-2 border-r border-slate-100">
                                          <span className={`px-2 py-0.5 border text-[9px] font-bold uppercase tracking-wider rounded-md ${h.transaction_type.includes('IN') ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-50 text-slate-700 border-slate-200'}`}>
                                             {h.transaction_type}
                                          </span>
                                       </td>
                  ...
                  <div className="px-4 py-2.5 bg-slate-50 border-t border-slate-200 flex justify-between items-center text-[9px] font-bold text-slate-400 uppercase tracking-wider shrink-0">
                     ...
                     <button
                        onClick={() => setShowHistory(false)}
                        className="bg-[#1d5f84] hover:bg-[#154662] text-white text-xs font-bold px-3 py-1.5 rounded-md transition shadow-sm uppercase tracking-wider"
                     >Close Audit</button>
                  </div>
               </div>
            </div>
