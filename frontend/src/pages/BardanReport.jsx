...
export default function BardanReport() {
  ...
  const handlePrint = () => {
    ...
    const rowsHtml = visibleRows.map((row) => {
      ...
      return `<tr>
        <td style="border:1.5px solid #000;padding:5px 8px;text-align:center;font-size:10px"><strong>#${billNo}</strong></td>
        <td style="border:1.5px solid #000;padding:5px 8px;font-size:10px"><strong>${name}</strong><br/><small style="color:#64748b">${memCodeFormatted}</small></td>
        <td style="border:1.5px solid #000;padding:5px 8px;font-size:10px">${item}</td>
        <td style="border:1.5px solid #000;padding:5px 8px;text-align:center;font-size:10px">${fmtDate(row.entry_date, isGu)}</td>
        <td style="border:1.5px solid #000;padding:5px 8px;text-align:center;font-size:10px">${row.quality_class || '-'}</td>
        <td style="border:1.5px solid #000;padding:5px 8px;text-align:right;font-size:10px"><strong>${fmtNum(row.returned_bags, 0, isGu)}</strong></td>
        <td style="border:1.5px solid #000;padding:5px 8px;text-align:right;font-size:10px"><strong>${fmtNum(row.total_kg, 2, isGu)}</strong></td>
        <td style="border:1.5px solid #000;padding:5px 8px;text-align:right;font-size:10px"><strong>${fmtNum(row.net_quintal, 2, isGu)}</strong></td>
      </tr>`;
    }).join('');
    ...
    win.document.write(`<html><head>
      ...
      <div style="border:1.5px solid #000;overflow:hidden;">
        <div style="border-bottom:1.5px solid #000;padding:12px;text-align:center;font-size:18px;font-weight:bold">${cName}</div>
        <div style="border-bottom:1.5px solid #000;padding:8px;text-align:center;font-size:14px;font-weight:bold">${labels.title}</div>
        <div style="border-bottom:1.5px solid #000;padding:8px 12px;display:flex;justify-content:space-between;font-size:12px;font-weight:bold">
        ...
        <table style="width:100%;border-collapse:collapse">
          <thead><tr>
            <th style="border:1.5px solid #000;padding:6px 8px;font-size:10px;background:#fff">${labels.bill} #</th>
            <th style="border:1.5px solid #000;padding:6px 8px;font-size:10px;background:#fff">${labels.member}</th>
            <th style="border:1.5px solid #000;padding:6px 8px;font-size:10px;background:#fff">${labels.item}</th>
            <th style="border:1.5px solid #000;padding:6px 8px;font-size:10px;background:#fff;text-align:center">${labels.date}</th>
            <th style="border:1.5px solid #000;padding:6px 8px;font-size:10px;background:#fff;text-align:center">${labels.class}</th>
            <th style="border:1.5px solid #000;padding:6px 8px;font-size:10px;background:#fff;text-align:right">${labels.bardan}</th>
            <th style="border:1.5px solid #000;padding:6px 8px;font-size:10px;background:#fff;text-align:right">${labels.weight} (kg)</th>
            <th style="border:1.5px solid #000;padding:6px 8px;font-size:10px;background:#fff;text-align:right">${labels.netQuintal}</th>
          </tr></thead>
          <tbody>${rowsHtml}</tbody>
          <tfoot><tr>
            <td colspan="5" style="border:1.5px solid #000;padding:6px 8px;font-size:11px;font-weight:bold;text-align:right">${labels.totals}</td>
            <td style="border:1.5px solid #000;padding:6px 8px;font-size:11px;font-weight:bold;text-align:right">${fmtNum(totalBardan, 0, isGu)}</td>
            <td style="border:1.5px solid #000;padding:6px 8px;font-size:11px;font-weight:bold;text-align:right">${fmtNum(totalWeight, 2, isGu)}</td>
            <td style="border:1.5px solid #000;padding:6px 8px;font-size:11px;font-weight:bold;text-align:right">${fmtNum(totalQty, 2, isGu)}</td>
          </tr></tfoot>
        </table>
      </div>
    </body></html>`);
      ...
      <div className="max-w-[1600px] mx-auto px-4 py-4 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-none flex flex-col justify-between">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{labels.totalEntries}</span>
            <span className="text-[13px] font-bold text-slate-800 mt-1">{isGu ? toGujaratiDigits(visibleRows.length) : visibleRows.length}</span>
          </div>
          <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-none flex flex-col justify-between">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{labels.jamaBardan}</span>
          ...
          <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-none flex flex-col justify-between">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{labels.totalWeight}</span>
          ...
          <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-none flex flex-col justify-between">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{labels.netQuintal}</span>
                  ...
                  <tr key={row.id} className="hover:bg-slate-50/75 transition-colors cursor-pointer" onClick={() => setSelectedEntry(row)}>
                    ...
                    <td className="px-3.5 py-2 border-r border-slate-100">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[9px] font-mono text-slate-400">{isGu ? `કોડ: ${toGujaratiDigits(row.member_code || '-')}` : `CODE: ${row.member_code || '-'}`}</span>
                      </div>
                    </td>
          ...
          <div className="w-screen max-w-sm bg-white border-l border-slate-200 flex flex-col shadow-none">
            <div className="px-4 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-800 uppercase tracking-wide">{labels.filterTitle}</span>
              </div>
                  ...
                  <div className="absolute top-[102%] left-0 right-0 bg-white border border-slate-200 rounded-md z-[110] mt-0.5 max-h-40 overflow-y-auto shadow-sm">
                    ...
                    {filteredMembers.map(member => (
                      <div key={member.id} onClick={() => handleSelectMember(member)} className="px-2.5 py-1 flex justify-between items-center cursor-pointer border-b border-slate-100 last:border-none hover:bg-slate-50">
                        ...
                        <span className="text-[8px] font-mono text-slate-400 font-semibold shrink-0">#{isGu ? toGujaratiDigits(member.member_code || member.id) : (member.member_code || member.id)}</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="px-4 py-3 bg-slate-50 border-t border-slate-200 flex gap-2">
              <button onClick={clearFilters} className="flex-1 px-3 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 text-xs font-bold rounded-md transition cursor-pointer uppercase tracking-wider">{labels.reset}</button>
              <button onClick={() => { setShowFiltersDrawer(false); fetchReportData(); }} className="flex-1 px-3 py-2 bg-[#1d5f84] hover:bg-[#154662] text-white text-xs font-bold rounded-md transition cursor-pointer uppercase tracking-wider">{labels.viewReport}</button>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="p-5 overflow-y-auto bg-white space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-slate-50 border border-slate-200 rounded-md p-3">
                  <div className="flex items-center gap-1.5 text-slate-400 text-[9px] font-bold uppercase tracking-wider"><Hash size={11} /> {isGu ? 'કોડ' : 'Code'}</div>
                  <p className="mt-1 text-xs font-mono font-bold text-[#1d5f84]">{isGu ? toGujaratiDigits(selectedEntry.member_code || '-') : (selectedEntry.member_code || '-')}</p>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-md p-3">
                  <div className="flex items-center gap-1.5 text-slate-400 text-[9px] font-bold uppercase tracking-wider"><Calendar size={11} /> {isGu ? 'તારીખ' : 'Date'}</div>
                  <p className="mt-1 text-xs font-mono font-bold text-slate-800">{fmtDate(selectedEntry.entry_date, isGu)}</p>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-md p-3">
                  <div className="flex items-center gap-1.5 text-slate-400 text-[9px] font-bold uppercase tracking-wider"><Package size={11} /> {isGu ? 'જમા' : 'Jama'}</div>
                  <p className="mt-1 text-xs font-mono font-bold text-[#1d5f84]">{fmtNum(selectedEntry.jama_qty || selectedEntry.returned_bags, 0, isGu)} {isGu ? 'બારદાન' : 'Bardan'}</p>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-md p-3">
                  <div className="flex items-center gap-1.5 text-slate-400 text-[9px] font-bold uppercase tracking-wider"><Scale size={11} /> {labels.weight}</div>
                  <p className="mt-1 text-xs font-mono font-bold text-slate-800">{fmtNum(selectedEntry.total_kg, 2, isGu)}</p>
                </div>
              </div>
