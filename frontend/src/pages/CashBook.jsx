  ...
  const handlePrint = () => {
    ...
    const rows = filteredEntries.map((entry, i) => `
      <tr style="background:${i % 2 === 0 ? '#fff' : '#f8fafc'}">
        ...
        <td>${entry.description} ${entry.notes ? '<br/><span style="color:#64748b;font-size:12px;">' + entry.notes + '</span>' : ''}</td>
    ...
    win.document.write(`<html><head><title>Cash Book Registry</title>
      <style>*{margin:0;padding:0;box-sizing:border-box}
        ...
        .logo-bar{background:#2563eb;color:#fff;padding:12px 15px;display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;border-radius:0}
        .logo-bar h1{font-size:12px;font-weight:900;text-transform:uppercase}
        .logo-bar span{font-size:12px;color:#dbeafe}
        h2{font-size:16px;font-weight:bold;text-transform:uppercase;margin-bottom:4px;color:#18181b}
        p.sub{font-size:12px;color:#71717a;margin-bottom:12px;text-transform:uppercase;letter-spacing:0.5px}
        hr{border:none;border-top:1px solid #e2e8f0;margin:12px 0}
        table{width:100%;border-collapse:collapse;border:1px solid #18181b}
        thead tr{background:#2563eb;color:#fff}
        th{padding:12px;font-size:12px;font-weight:bold;text-transform:uppercase;background:#f4f4f5;border:1px solid #18181b;text-align:left}
        td{padding:6px 12px;border-bottom:1px solid #f1f5f9;font-size:12px;border:1px solid #e4e4e7}
      ...
      <div style="display:flex; justify-content:space-between; margin-bottom:15px; background:#f8fafc; padding:10px 15px; border-radius:6px; border:1px solid #e2e8f0;">
         <div style="text-align:center;">
            <div style="font-size:12px; color:#64748b; font-weight:bold; text-transform:uppercase;">Total Receipts (Jama)</div>
            <div style="font-size:16px; color:#059669; font-weight:bold;">${summary.total_in.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</div>
         </div>
         <div style="text-align:center;">
            <div style="font-size:12px; color:#64748b; font-weight:bold; text-transform:uppercase;">Total Payments (Udhar)</div>
         ...
         <div style="text-align:center;">
            <div style="font-size:12px; color:#64748b; font-weight:bold; text-transform:uppercase;">Net Cash Balance</div>
    ...
    <div className="min-h-screen bg-zinc-100 p-6 font-sans text-zinc-900 animate-in fade-in duration-300">
      <div className="max-w-[1400px] mx-auto bg-white border border-zinc-300 shadow-sm p-5 space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-zinc-300 pb-4 gap-4">
          <div>
            ...
            <p className="text-sm font-mono text-zinc-500 mt-0.5 uppercase tracking-wider select-none">Financial Management / Treasury</p>
          </div>

          <div className="flex items-center gap-3 w-full md:w-auto">
            <button
              onClick={() => { setEditingId(null); setModalType('credit'); setModalOpen(true); }}
              className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 border border-emerald-500 text-white text-sm font-bold px-4 py-2 rounded-none transition shadow-sm select-none uppercase"
            >
              <ArrowUpRight size={16} /> JAMA ENTRY
            </button>
            <button
              onClick={() => { setEditingId(null); setModalType('debit'); setModalOpen(true); }}
              className="flex items-center gap-1.5 bg-red-600 hover:bg-red-700 border border-red-500 text-white text-sm font-bold px-4 py-2 rounded-none transition shadow-sm select-none uppercase"
            >
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center border border-zinc-300 bg-white px-2 py-1.5 select-none gap-2">
                ...
                <input
                  ...
                  className="bg-transparent text-sm text-zinc-800 outline-none font-mono"
                />
              ...
              <button
                ...
                className="flex items-center gap-1 bg-white border border-zinc-300 hover:bg-zinc-50 text-zinc-700 text-sm font-bold px-3 py-1.5 select-none uppercase transition shadow-sm"
              >
          ...
          <div className="overflow-x-auto flex-1 bg-white">
            {loading ? (
              <div className="py-20 text-center">
                ...
                <p className="text-sm font-bold text-zinc-400 font-mono uppercase tracking-widest">Synchronizing Treasury Streams...</p>
              </div>
            ) : filteredEntries.length === 0 ? (
              <div className="py-20 text-center text-zinc-400 font-bold font-mono text-sm uppercase tracking-widest">
                No Transaction Nodes Isolated
              </div>
            ) : (
              <table className="w-full text-left border-collapse font-sans text-sm select-none">
                <thead>
                  <tr className="bg-zinc-50 border-b border-zinc-300 text-zinc-600 font-mono text-sm">
                ...
                <tbody className="divide-y divide-zinc-200">
                  {filteredEntries.map((entry, idx) => (
                    <tr key={entry.id} className="hover:bg-zinc-50/60 font-mono text-sm transition-colors">
                      ...
                      <td className="px-4 py-2 border-r border-zinc-200">
                        <div className="flex flex-col">
                          ...
                          <span className={`inline-flex w-fit px-1.5 py-0.5 mt-0.5 text-[12px] font-bold border ${entry.reference_type === 'sale' ? 'bg-blue-50 border-blue-200 text-blue-600' :
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-[1000] animate-in slide-in-from-right-10 duration-300">
          <div className={`px-4 py-3 shadow-2xl border flex items-center gap-3 ${toast.type === 'error' ? 'bg-red-50 border-red-200 text-red-800' : 'bg-blue-600 border-blue-500 text-white'
            }`}>
