  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-12 animate-in fade-in duration-700">
      <div className="max-w-[1600px] mx-auto px-8">
        ...
        {/* Status Indicators */}
        {message && (
          <div className={`mb-8 p-5 rounded-3xl flex items-center gap-4 animate-in slide-in-from-top duration-300 border-l-[6px] ${
                ...
                {['all', 'active', 'inactive'].map((filt) => (
                  <button
                    ...
                    className={`px-6 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${
                      statusFilter === filt ? 'bg-white text-slate-900 shadow-xl scale-105' : 'text-slate-400 hover:text-slate-600'
                    }`}
                  >
                    {filt}
                  </button>
                 ...
                 <tbody className="divide-y divide-slate-100/50">
                   {filteredMembers.map((member) => (
                     <tr key={member.id} className="group hover:bg-white/60 transition-all duration-500">
                       ...
                       <td className="px-10 py-8">
                         <div className={`inline-flex items-center gap-3 px-5 py-2.5 rounded-2xl text-[9px] font-black uppercase tracking-[0.2em] transition-all border shadow-sm ${
                           member.is_active ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-400 border-rose-100'
                         }`}>
