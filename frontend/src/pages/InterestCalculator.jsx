        ...
        <head>
          ...
          <style>
            ...
            * { margin:0; padding:0; box-sizing:border-box; }
            ...
            .pdf-table th, .pdf-table td { border:1.5px solid #000 !important; padding:7px 12px; font-size:12px; color:#000; }
        ...
        <div className="flex flex-wrap items-center gap-4 bg-slate-50 border border-slate-200 rounded-lg p-4 select-none">
          <div className="flex flex-col gap-1.5 w-full sm:w-auto">
            ...
            <div className="flex items-center bg-white border border-slate-200 rounded-md px-2.5 h-8 focus-within:border-[#1d5f84] focus-within:ring-1 focus-within:ring-[#1d5f84] transition w-full sm:w-28">
              ...
              <input
                ...
                className="text-sm font-bold text-slate-700 bg-transparent border-none p-0 focus:ring-0 outline-none w-full font-mono"
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5 w-full sm:w-auto">
            ...
            <div className="flex items-center bg-white border border-slate-200 rounded-md px-2.5 h-8 focus-within:border-[#1d5f84] focus-within:ring-1 focus-within:ring-[#1d5f84] transition w-full sm:w-36">
              ...
              <select
                ...
                className="text-sm font-bold text-slate-700 bg-transparent border-none p-0 focus:ring-0 outline-none w-full appearance-none pr-4"
              >
          ...
          <div className="flex items-end pt-5 w-full sm:w-auto">
            <button
              ...
              className={`h-8 flex items-center gap-1.5 px-4 text-sm font-bold text-white transition rounded-md cursor-pointer select-none shadow-none border w-full sm:w-auto ${isComputed
          ...
          <div className="flex items-center gap-3 ml-auto">
            <div className="flex flex-col justify-between bg-white border border-slate-200 rounded-lg px-4 py-2 h-14 shadow-none min-w-[140px]">
              <span className="text-[12px] font-bold text-slate-400 uppercase tracking-wider">{t('interestCalculator.activePrincipal')}</span>
            ...
            <div className="flex flex-col justify-between bg-white border border-slate-200 rounded-lg px-4 py-2 h-14 shadow-none min-w-[140px]">
              <span className="text-[12px] font-bold text-slate-400 uppercase tracking-wider">{t('interestCalculator.accruedInterest')}</span>
        ...
        <div className="bg-white border border-slate-200 rounded-lg overflow-hidden flex flex-col min-h-[500px] shadow-none select-none">
          ...
          <div className="px-3.5 py-2 bg-slate-50 border-b border-slate-200 flex items-center justify-between flex-wrap gap-2 select-none">
            <div className="flex items-center gap-2">
              <span className={`text-sm font-extrabold text-slate-700 uppercase tracking-wider ${i18n.language === 'gu' ? 'font-prompt-sm' : ''}`}>
            ...
            <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto">
              <div className="h-7 flex items-center gap-1.5 px-2 bg-white border border-slate-200 rounded-md focus-within:border-[#1d5f84] focus-within:ring-1 focus-within:ring-[#1d5f84] w-full sm:w-64 transition">
                ...
                <input
                  ...
                  className="bg-transparent border-none outline-none text-[12px] text-slate-700 placeholder:text-slate-400 w-full font-bold font-prompt-sm"
                />
              </div>
              <button
                onClick={handlePrint}
          ...
          <div className="flex-1 overflow-x-auto bg-white select-none">
            <table className="w-full text-left border-collapse font-sans text-sm select-none">
                ...
                {loading ? (
                  <tr>
                    <td colSpan="9" className="py-20 text-center">
                      ...
                      <p className="text-sm font-bold text-slate-400 uppercase tracking-widest font-prompt-sm">{t('interestCalculator.processing')}</p>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td colSpan="9" className="py-20 text-center">
                      ...
                      <p className="text-sm font-bold text-slate-400 uppercase tracking-widest font-prompt-sm">{t('interestCalculator.noNodes')}</p>
                    </td>
                  </tr>
                ) : (
                      ...
                      >
                        <td className="py-3 px-4 border-r border-slate-100">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-[12px] font-bold text-slate-800 tracking-tight uppercase font-prompt-sm">{getDisplayName(row)}</p>
                              <p className="text-[12px] font-bold text-slate-400 uppercase tracking-widest font-mono">CODE: {row.member_code}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-3 px-4 border-r border-slate-100">
                          <p className="text-[12px] font-bold text-slate-600 truncate max-w-[200px] font-prompt-sm">
                        ...
                        <td className="py-3 px-4 border-r border-slate-100 text-right font-bold text-rose-600 font-mono text-[12px]">
                          ₹{parseFloat(row.debit || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                        </td>
                        <td className="py-3 px-4 border-r border-slate-100 text-right font-bold text-emerald-600 font-mono text-[12px]">
                          ₹{parseFloat(row.credit || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                        </td>
                        <td className="py-3 px-4 border-r border-slate-100 text-right font-bold text-slate-800 font-mono text-[12px]">
                        ...
                        <td className="py-3 px-4 border-r border-slate-100 text-center">
                          <span className="text-[12px] font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200 font-mono">
                            {row.elapsedDays} D
                          </span>
                        </td>
                        <td className="py-3 px-4 border-r border-slate-100 text-center text-[12px] font-bold text-slate-600 font-mono">
                          {isComputed ? `${parseFloat(globalRate) || row.interest_percent}%` : '—'}
                        </td>
                        <td className="py-3 px-4 border-r border-slate-100 text-right font-black text-[#1d5f84] font-mono text-[12px]">
                          ...
                          <div className="flex justify-between items-center w-full gap-2">
                            {isComputed && parseFloat(calculateYield(row)) > 0 && (
                              <button
                                ...
                                className="h-5 flex items-center px-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-[#1d5f84] hover:text-[#154662] transition rounded font-bold text-[12px] uppercase tracking-wider select-none cursor-pointer"
                                title="Settle Interest"
                              >
                                SETTLE
                              </button>
                            )}
                            <span className="text-[12px] font-black text-slate-800 bg-slate-50 px-2 py-0.5 border border-slate-200 rounded font-mono ml-auto">
                      ...
                      {expandedRows.has(row.member_id) && row.entries && row.entries.map((entry, eIdx) => (
                        <tr key={`${row.member_id}-sub-${eIdx}`} className="bg-slate-50/30 border-t border-slate-100">
                          <td className="py-2.5 px-4 pl-10 border-r border-slate-100">
                            <p className="text-[12px] font-bold text-slate-400 uppercase tracking-widest font-prompt-sm">Node {eIdx + 1}</p>
                          </td>
                          <td className="py-2.5 px-4 border-r border-slate-100">
                            <p className="text-[12px] font-bold text-slate-500 truncate max-w-[200px] font-prompt-sm">{entry.description}</p>
                            <p className="text-[12px] text-slate-400 font-mono"># {entry.reference_no}</p>
                          </td>
                          <td className="py-2.5 px-4 border-r border-slate-100 text-right text-sm text-rose-600 font-mono">₹{parseFloat(entry.debit || 0).toFixed(2)}</td>
                          <td className="py-2.5 px-4 border-r border-slate-100 text-right text-sm text-emerald-600 font-mono">₹{parseFloat(entry.credit || 0).toFixed(2)}</td>
                          <td className="py-2.5 px-4 border-r border-slate-100 text-right text-sm font-bold text-slate-600 font-mono">₹{parseFloat(entry.principal).toFixed(2)}</td>
                          ...
                          <td className="py-2.5 px-4 border-r border-slate-100 text-right font-bold text-[#1d5f84] text-sm font-mono">₹{isComputed ? parseFloat(calculateYield(entry)).toFixed(2) : '0.00'}</td>
                          <td className="py-2.5 px-4 text-right font-bold text-slate-400 text-sm font-mono">₹{parseFloat(entry.principal).toFixed(2)}</td>
                        </tr>
                      ))}
                    </React.Fragment>
                  ))
            ...
            <form onSubmit={handleSettleSubmit} className="p-5 space-y-4">
              ...
              <div className="flex flex-col gap-1.5 font-prompt-sm">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white border border-slate-200 hover:border-slate-300 focus:border-[#1d5f84] focus:ring-1 focus:ring-[#1d5f84] transition rounded-md outline-none text-sm text-slate-700 font-bold font-mono"
                />
              </div>
              ...
              <div className="flex flex-col gap-1.5 font-prompt-sm">
                ...
                <div className="flex flex-col gap-2">
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white border border-slate-200 hover:border-slate-300 focus:border-[#1d5f84] focus:ring-1 focus:ring-[#1d5f84] transition rounded-md outline-none text-sm text-slate-700 font-bold"
                  />
                  <div className="flex flex-wrap gap-1.5">
                    {['interestAccrued', 'interestSettled', 'interestAdjustment', 'openingBalanceInterest'].map(key => (
                      <button
                        ...
                        className="text-[12px] font-bold bg-slate-50 hover:bg-slate-100 px-2 py-1 border border-slate-200 text-slate-600 rounded transition cursor-pointer"
                      >
              ...
              <div className="flex gap-2 pt-2 font-prompt-sm">
                <button
                  type="button"
                  onClick={() => setSettleModalRow(null)}
                  className="flex-1 h-8 flex items-center justify-center bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 hover:text-slate-800 font-bold text-sm uppercase tracking-wider rounded-md cursor-pointer transition"
                >
                  {t('common.cancel')}
                </button>
                <button
                  type="submit"
                  className="flex-2 h-8 flex items-center justify-center text-white bg-[#1d5f84] hover:bg-[#154662] border border-[#1d5f84] font-bold text-sm uppercase tracking-wider rounded-md cursor-pointer transition shadow-none"
                >
