    ...
    <div className="p-8 scroller-airy overflow-y-auto h-[calc(100vh-80px)]">
      ...
      <div className="flex justify-between items-center mb-8">
         ...
         <button 
           ...
           className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl flex items-center gap-2 font-black text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-100 transition-all active:scale-95"
         >
      ...
      <div className="grid grid-cols-12 gap-8">
         ...
         <div className="col-span-12 space-y-4 text-slate-800">
            ...
            {loading ? (
               <div className="h-64 flex flex-col items-center justify-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 italic text-slate-300">
               ...
               <div className="h-64 flex flex-col items-center justify-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 italic text-slate-300 text-slate-800">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
                  {filtered.map(n => (
                     <div key={n.id} className="bg-white p-5 rounded-3xl border border-slate-50 hover:border-indigo-100 transition-all shadow-sm group">
                        <div className="flex justify-between items-start">
                           <div className="flex gap-4">
                              <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center text-indigo-50/20 group-hover:bg-indigo-600 group-hover:text-white transition-all">
                                 <FileText size={20} />
                              </div>
                           ...
                           <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => handleEdit(n)} className="p-2.5 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all"><Edit2 size={14}/></button>
                              <button onClick={() => handleDelete(n.id)} className="p-2.5 bg-rose-50 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all"><Trash2 size={14}/></button>
                           </div>
                        </div>
                     </div>
                  ))}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-500 text-slate-800">
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                 {error && (
                    <div className="bg-rose-50 text-rose-500 p-4 rounded-2xl flex items-center gap-3 border border-rose-100 animate-pulse">
                       <AlertCircle size={18} />
                 ...
                 <div className="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input 
                           ...
                           className="w-full bg-[#F8FAFC] border border-slate-100 rounded-xl p-3 text-xs font-black text-slate-700 outline-none focus:border-indigo-200 transition-all uppercase placeholder:italic"
                    ...
                    <div>
                        ...
                        <textarea 
                           ...
                           className="w-full bg-[#F8FAFC] border border-slate-50 rounded-2xl p-5 text-sm font-bold text-slate-700 focus:bg-white focus:border-indigo-200 outline-none transition-all resize-none shadow-inner italic"
                 ...
                 <div className="flex gap-4 pt-4">
                    <button 
                      ...
                      className="flex-1 bg-slate-50 text-slate-400 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-slate-100 hover:text-slate-600 transition-all active:scale-95"
                    >
                      Abort
                    </button>
                    <button 
                      type="submit" 
                      className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95"
                    >
