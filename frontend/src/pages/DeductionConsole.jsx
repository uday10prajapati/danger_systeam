   ...
   const handleExecuteBatch = async (providedIdentities = null) => {
      try {
         ...
         if (res.data.success) {
            ...
            loadIdentities(); // Refresh all balances to show settled state
         }
      } catch (e) {
         ...
         >
            <div className="flex items-center gap-3">
                <button
                   ...
                   className="px-4 py-2.5 bg-white text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50 transition-all font-bold text-xs flex items-center gap-2 shadow-sm disabled:opacity-50"
                >
                   <TrendingUp size={15} /> Smart Fill
                </button>
                   ...
                   onClick={async () => {
                      ...
                      try {
                         ...
                         if (updatedIdentities.length > 0) {
                            ...
                            alert('No outstanding balances found to pay.');
                         }
                      } catch (err) { console.error(err); } finally { setIsSmartFilling(false); }
                   }}
                   disabled={isSmartFilling}
                   className="px-4 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-all font-bold text-xs flex items-center gap-2 shadow-sm disabled:opacity-50"
                >
                   <CheckCircle size={15} /> Smart Pay
                </button>
                  ...
                  <table className="w-full text-left">
                     <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                           ...
                           <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Auto-Calc</th>
                     ...
                     <tbody className="divide-y divide-slate-100">
                        {selectedIdentities.map((item, idx) => (
                           <tr key={`${item.type}-${item.id}-${idx}`} className="hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="px-6 py-4">
                                 <button 
                                    ...
                                    className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all ${
                                       item.is_auto !== false ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
                                    }`}
                                 >
      ...
      {showDeductionModal && (
         <div className="fixed inset-0 z-[100] flex justify-center items-center p-4">
            ...
            <div className="relative w-full max-w-3xl bg-white shadow-2xl flex flex-col border-2 border-slate-400 rounded-sm" style={{ maxHeight: '90vh' }}>
               ...
               <div className="bg-[#F8FAFC] border-b border-slate-200 px-6 py-3 space-y-3 shrink-0">
                  <div className="grid grid-cols-2 gap-4">
                     <div className="space-y-1">
                        <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Voucher No</label>
                        <div className="px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-lg text-xs font-black text-slate-500">000001</div>
                     </div>
                     <div className="space-y-1">
                        <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">Process Date</label>
                        <input type="date" value={deductionPayload.date}
                           onChange={e => setDeductionPayload(p => ({ ...p, date: e.target.value }))}
                           className="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-700 outline-none focus:border-blue-500 transition-all" />
                     </div>
                  </div>
                   <div className="flex justify-end gap-2 pt-1">
                      <button 
                         ...
                         className="text-[10px] font-bold text-slate-600 hover:text-slate-800 uppercase tracking-widest flex items-center gap-1.5 px-3 py-1 bg-slate-100 rounded-md transition-all border border-slate-200"
                      >
                      ...
                      <button 
                         ...
                         className="text-[10px] font-bold text-white bg-emerald-600 hover:bg-emerald-700 uppercase tracking-widest flex items-center gap-1.5 px-3 py-1 rounded-md transition-all shadow-md shadow-emerald-100"
                      >
                         <CheckCircle size={12} /> Smart Pay
                      </button>
                   </div>
                  <div className="space-y-1">
                     <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest ml-1">{isSubledger ? 'Sabhasad Identity' : 'Narration / Description'}</label>
                     <div className="flex gap-2">
                        <input
                           ...
                           className="w-20 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-black text-slate-700 text-center outline-none focus:border-blue-500 transition-all"
                           placeholder="Code"
                        />
                        <input
                           type="text"
                           ...
                           className="flex-1 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-bold text-slate-700 outline-none focus:border-blue-500 transition-all"
               ...
               {deductionPayload.target_identifier && (
                  <div className="bg-white border-b border-slate-100 px-6 py-2 flex items-center justify-between shrink-0 gap-4">
                     <div className="flex items-center gap-3">
               ...
               <div className="flex flex-col overflow-hidden flex-1" style={{ minHeight: 0 }}>
                  <div className="grid shrink-0 border-b-2 border-slate-400 bg-slate-200"
                     ...
                     {['No.', 'Code', 'Account Name', 'Balance', 'Amount'].map((h, i) => (
                        <div key={h} className={`py-1 text-[10px] font-black text-slate-700 uppercase ${i < 4 ? 'border-r border-slate-400' : ''} ${i >= 2 ? 'text-right px-3' : 'text-center'}`}>{h}</div>
                     ))}
                  </div>
                     ...
                     {selectedIdentities.length === 0 ? (
                        ...
                        return (
                           <div key={key}
                              ...
                              style={{ gridTemplateColumns: '36px 72px 1fr 110px 100px' }}>
                              <div className={`border-r py-1 text-center text-[10px] font-bold ${isActive ? 'border-blue-500 text-blue-100' : 'border-slate-200 text-slate-400'}`}>{idx + 1}</div>
                              <div className={`border-r py-1 text-center text-[10px] font-mono font-black ${isActive ? 'border-blue-500 text-white' : 'border-slate-200 text-slate-700'}`}>{String(item.code || '').padStart(4, '0')}</div>
                              <div className={`border-r px-3 py-1 text-[10px] font-bold truncate ${isActive ? 'border-blue-500 text-white' : 'border-slate-200 text-slate-800'}`}>{item.name}</div>
                              <div className={`border-r px-3 py-1 flex flex-col items-end justify-center ${isActive ? 'border-blue-500' : 'border-slate-200'}`}>
                                 <div className={`text-[10px] font-mono font-black ${isActive ? 'text-white' : bal < 0 ? 'text-rose-600' : bal > 0 ? 'text-emerald-600' : 'text-slate-600'}`}>
                                    {bal > 0 ? '+' : bal < 0 ? '-' : ''}{Math.abs(bal).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                                 </div>
                                 {isActive && deducted !== 0 && Math.abs(closing) > 0.01 && (
                                    <div className="text-[9px] font-mono font-black italic mt-0.5 text-blue-100">
         ...
         <div className="fixed inset-0 z-[110] flex justify-center items-center p-4">
            ...
            <div className="relative w-full max-w-lg bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
               ...
               <div className="p-6 flex gap-2 border-b border-slate-100">
                  <button onClick={() => setIdentityTab('member')} className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${identityTab === 'member' ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>Members</button>
                           ...
                           <div key={id} onClick={() => toggleIdentitySelection(id, identityTab, name, code)} className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-all border ${isSelected ? 'bg-blue-50 border-blue-200' : 'hover:bg-slate-50 border-transparent'}`}>
                              <div className="flex items-center gap-3">
                                 <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isSelected ? 'bg-blue-600 text-white shadow-sm' : 'bg-slate-100 text-slate-400'}`}>
                                    {isSelected ? <CheckCircle size={16} /> : (identityTab === 'member' ? <User size={16} /> : <Layout size={16} />)}
                                 </div>
                                 <div>
