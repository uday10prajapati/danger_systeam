  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-12 animate-in fade-in duration-700">
      <div className="max-w-[1600px] mx-auto px-8">
        ...
        {/* Dynamic Filters */}
        <div className="bg-white/80 backdrop-blur-xl p-6 rounded-lg border border-slate-100 shadow-sm mb-6 flex flex-wrap items-center gap-4">
        ...
        <div className="bg-white rounded-lg border border-slate-100 shadow-2xl overflow-hidden min-h-[600px] flex flex-col">
          <TableHeading
            ...
            iconColor="slate"
            title="Permanent Transaction Node Archive"
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-100">
                    ...
                    <th className="px-6 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Ref. SR</th>
                    <th className="px-6 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-wider">Item</th>
                    <th className="px-6 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-wider text-right">Net Volume</th>
                    <th className="px-6 py-5 text-[11px] font-bold text-slate-400 uppercase tracking-wider text-right">Rate (Qt)</th>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-bold text-slate-400 font-mono">#{row.member_code}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <span className="text-blue-600 font-black text-sm tracking-tight font-mono">#{row.sr_no}</span>
                        <p className="text-[9px] font-bold text-slate-400 uppercase leading-none mt-1">{new Date(row.entry_date).toLocaleDateString()}</p>
                      </td>
                      <td className="px-6 py-5">
                        <p className="text-sm font-bold text-slate-800 uppercase tracking-tight">{row.item_name}</p>
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{row.book_type}</p>
                      </td>
                      <td className="px-6 py-5 text-right">
                        <span className="font-black text-slate-800 text-sm">{parseFloat(row.net_quintal).toFixed(2)}</span>
                        <span className="text-[10px] text-slate-400 uppercase ml-1">Qt</span>
                      </td>
                      <td className="px-6 py-5 text-right font-black text-blue-600 text-sm">
                        ₹{parseFloat(row.rate).toFixed(2)}
                      </td>
                      <td className="px-6 py-5 text-right font-black text-emerald-600 text-base bg-emerald-50/30">
                        ₹{parseFloat(row.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                      </td>
                      <td className="px-6 py-5">
                        <div className="flex flex-col gap-1">
                          {row.vehicle_no && (
                            <div className="flex items-center gap-1.5 text-[10px] font-black text-slate-500 uppercase tracking-widest bg-slate-100 px-2 py-0.5 rounded-lg w-fit">
                              <Layout size={10} /> {row.vehicle_no}
                            </div>
                          )}
                          <p className="text-[10px] font-medium text-slate-400 line-clamp-1 truncate w-48">{row.remark || '—'}</p>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <div className="flex justify-center">
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-600 text-[9px] font-black uppercase tracking-widest rounded-full border border-emerald-100">
          ...
          <div className="px-8 py-5 border-t border-slate-100 flex justify-between items-center bg-slate-50/50">
            <div className="flex items-center gap-6">
              <div className="flex flex-col">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Total Net Volume</p>
                <p className="text-xl font-black text-slate-800 leading-none">
                  {filteredEntries.reduce((acc, curr) => acc + parseFloat(curr.net_quintal || 0), 0).toFixed(2)}
                  <span className="text-xs font-bold text-slate-400 ml-1">Qt</span>
                </p>
              </div>
