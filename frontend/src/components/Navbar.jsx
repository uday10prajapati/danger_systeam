  ...
  if (!group.children) {
    return (
      <button
        ...
        className={`w-full flex items-center gap-3 px-3 py-2 text-sm font-bold transition select-none tracking-wide ${active
  ...
  return (
    <div className="space-y-1">
      <button
        ...
        className={`w-full flex items-center justify-between px-3 py-2 text-sm font-bold transition select-none tracking-wide rounded-md ${isChildActive && !isOpen
          ...
          {group.children.map(child => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-1.5 text-sm text-left transition select-none rounded-md ${childActive
        ...
        <div className="flex items-center gap-3 overflow-hidden cursor-pointer" onClick={() => navigate('/dashboard')}>
          ...
          {!isCollapsed && (
            <div className="flex flex-col">
              ...
              <span className="text-[12px] font-bold text-emerald-500 uppercase tracking-widest">{backendStatus}</span>
            </div>
          )}
        </div>
      </div>
            ...
            {!isCollapsed && (
              <>
                <div className="flex-1 text-left truncate">
                  <p className="text-[12px] font-bold text-slate-700 truncate">{currentUser.full_name_gu || currentUser.username}</p>
                </div>
          ...
          {showProfile && (
            <div className={`absolute bottom-full mb-2 bg-white border border-slate-200 rounded-md shadow-lg py-1 animate-in fade-in zoom-in-95 duration-100 font-sans z-50 ${isCollapsed ? 'left-0 w-48 origin-bottom-left' : 'right-0 w-full origin-bottom'}`}>
              <div className="px-3 py-2 mb-1 border-b border-slate-100">
                <p className="text-sm font-bold text-slate-800">{(currentUser.full_name_gu || currentUser.username || 'ADMIN')}</p>
                <p className="text-[10px] text-slate-500 mt-0.5 uppercase tracking-wide">{currentUser.role || 'ADMIN'}</p>
              </div>
              <button onClick={() => { localStorage.removeItem('user'); navigate('/login') }} className="w-full flex items-center gap-2 px-3 py-2 text-rose-600 hover:bg-rose-50 transition font-bold text-sm uppercase select-none">
