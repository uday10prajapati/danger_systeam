...
const BackupRestoreManager = () => {
    ...
    return (
        <div className="bg-white border border-slate-200 rounded-lg overflow-hidden flex flex-col shadow-none font-sans select-none">
                ...
                <div className="bg-white border border-slate-200 rounded-lg p-4 flex flex-col justify-between group hover:border-[#1d5f84] transition-colors shadow-none">
                    <div>
                        <div className="flex items-center gap-2 mb-2 text-[#1d5f84]">
                            <Database size={16} />
                            <h3 className="font-bold text-[12px] uppercase tracking-wider text-slate-800">{t('settings.backup')}</h3>
                        </div>
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="h-8 flex items-center justify-center gap-1.5 bg-[#1d5f84] hover:bg-[#154662] disabled:bg-slate-300 disabled:border-slate-300 border border-[#1d5f84] text-white text-[12px] font-bold rounded-md transition shadow-none cursor-pointer uppercase tracking-wider w-full"
                    >
                ...
                <div className="bg-white border border-slate-200 rounded-lg p-4 flex flex-col justify-between group hover:border-rose-400 transition-colors shadow-none">
                    <div>
                        <div className="flex items-center gap-2 mb-2 text-rose-600">
                            <RotateCcw size={16} />
                            <h3 className="font-bold text-[12px] uppercase tracking-wider text-slate-800">{t('settings.restore')}</h3>
                        </div>
                        <p className="text-[10px] text-slate-500 font-semibold leading-relaxed mb-4">
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="h-8 flex items-center justify-center gap-1.5 bg-rose-600 hover:bg-rose-700 disabled:bg-slate-300 disabled:border-slate-300 border border-rose-600 text-white text-[12px] font-bold rounded-md transition shadow-none cursor-pointer uppercase tracking-wider w-full"
                    >
            ...
            {/* Status Messages */}
            {status && (
                <div className={`flex items-start gap-3 px-4 py-3 border-t text-[12px] font-bold uppercase tracking-wider ${status.type === 'error' ? 'bg-rose-50 border-rose-200 text-rose-700' :
                    ...
                    <div className="flex-1">
                        <p className="text-[12px] font-extrabold uppercase tracking-widest opacity-70 mb-0.5">{status.type === 'info' ? 'Processing' : status.type}</p>
                        <p className="font-sans normal-case text-sm">{status.message}</p>
                    </div>
