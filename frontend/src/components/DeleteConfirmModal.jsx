...
export default function DeleteConfirmModal({ isOpen, title, message, onConfirm, onCancel }) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      <div className="bg-white border border-zinc-400 rounded-none w-full max-w-sm shadow-lg relative z-10 overflow-hidden flex flex-col font-mono text-xs select-none">
        ...
        <div className="p-4 bg-white flex flex-col gap-3">
          ...
          <div className="flex gap-2 justify-end border-t border-zinc-100 pt-3 mt-1">
            <button
              onClick={onCancel}
              className="px-3.5 py-1.5 bg-white border border-zinc-300 hover:bg-zinc-50 text-zinc-700 font-bold transition select-none rounded-none text-xs"
            >
              CANCEL
            </button>
            <button
              onClick={onConfirm}
              className="px-4 py-1.5 bg-blue-600 border border-blue-500 hover:bg-blue-700 text-white font-bold transition select-none rounded-none text-xs"
            >
              DELETE
            </button>
          </div>
