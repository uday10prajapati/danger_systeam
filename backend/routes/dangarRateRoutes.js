...
router.post('/', async (req, res) => {
  try {
    const { company_id, financial_year, item_id, rate, bardan_deduction_rate } = req.body;
    
    const sql = `
      INSERT INTO dangar_rates (company_id, financial_year, item_id, rate, bardan_deduction_rate)
      VALUES (?, ?, ?, ?, ?)
    ...
    await query(sql, [company_id, financial_year, item_id, rate, bardan_deduction_rate]);
