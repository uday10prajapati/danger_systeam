...
// GET bardan entry by pavti number (includes grid items)
router.get('/by-pavti/:pavti', async (req, res) => {
  try {
    ...
    let sql = 'SELECT * FROM bardan_entry WHERE pavti_no = ?';
    const params = [pavti];
    ...
    const items = await query('SELECT * FROM bardan_items WHERE entry_id = ? ORDER BY id ASC', [entry.id]);
    ...
    console.error('Fetch bardan entry by pavti error:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
