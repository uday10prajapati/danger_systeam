...
let src = fs.readFileSync(path.resolve('frontend/src/pages/ItemRate.jsx'), 'utf8');
...
const fontLoader = `
   ...
   const handleExportPDF = async () => {
    ...
    const hdr = () => {
       ...
       doc.text('Item Rate Control List', W/2, 17, {align:'center'});
    ...
    const ftr = (pg, tot) => {
       ...
       doc.text(cName + ' - Item Rate Control List', M, H-9);
    ...
    doc.text('Item Rate Control List', M, y);
    ...
    autoTable(doc, {
       startY: y,
       head: [['Nomenclature', 'Code', 'Procurement Rate', 'Retail Rate', 'Status', 'Updated']],
       body: bodyRows,
    ...
    doc.save('Item_Rate_Control_List.pdf');
  };
`;

if (!src.includes('handleExportPDF')) {
    src = src.replace("const applyFilters = (ratesToFilter, search, status) => {", fontLoader + "\n  const applyFilters = (ratesToFilter, search, status) => {");
    
    // Check if jsPDF needs to be imported
    if (!src.includes("import jsPDF from 'jspdf';")) {
        src = src.replace("import axios from 'axios';", "import axios from 'axios';\nimport jsPDF from 'jspdf';\nimport autoTable from 'jspdf-autotable';");
    }
    
    if (!src.includes('Printer, FileText')) {
        src = src.replace('Printer,', 'Printer, FileText,');
        if (!src.includes('FileText')) {
            src = src.replace('from \'lucide-react\';', '  FileText,\n  Printer,\n} from \'lucide-react\';');
        }
    }
    ...
    fs.writeFileSync(path.resolve('frontend/src/pages/ItemRate.jsx'), src, 'utf8');
    console.log('ItemRate patched.');
} else {
    console.log('ItemRate already patched.');
}
